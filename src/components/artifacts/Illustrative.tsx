/* ────────────────────────────────────────────────────────────────────────────
   ...
   two things, and it says which ON ITSELF:
     ...
     <Sample />        the shape is drawn from numbers we made up. Used where a
                       chart illustrates a claim rather than reporting one.
     <Illustrative />  the content is a generic stand-in, not a real record.
                       Used for the mock result list, which must never be
                       mistaken for a screenshot of an actual search.
...
export function Illustrative({ children = 'Illustrative' }: { children?: string }) {
  return <span className="artifact-tag">{children}</span>
}
