...
export const metadata: Metadata = {
  ...
  // ── ICONS ────────────────────────────────────────────────────────────────
  // Every file referenced here is generated by scripts/gen-icons.mjs from the
  // portal's canonical mark geometry. Do not add a second icon source.
  //
  // This block previously declared FIVE competing icons, and the first one in
  // document order was `shortcut: '/icon-512.png'` — a rounded tile with
  // transparent corners at a size Google rejects (512 is not a multiple of 48).
  // That was the icon Google was picking, which is why the search result never
  // matched the tab. There is deliberately no `shortcut` entry now: it is a
  // legacy alias for `icon`, and Next emits it ahead of everything else.
  //
  // Order matters. The opaque PNG is listed first so crawlers take it; browsers
  // prefer image/svg+xml regardless of position, so tabs still get the adaptive
  // vector mark and keep matching the portal.
  icons: {
    icon: [
